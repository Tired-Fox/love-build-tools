...
function Boid:steeringBehavior(targetX, targetY, width, height)
	...
	if distanceToTarget < arrivalRadius then
		...
		targetVectorX = (targetVectorX * seekFactor - self.vx * arrivalFactor * slowingFactor)
			/ (seekFactor + arrivalFactor)
		targetVectorY = (targetVectorY * seekFactor - self.vy * arrivalFactor * slowingFactor)
			/ (seekFactor + arrivalFactor)
	else
